...
class DataGenerator(object):
    def __init__(self, params):
        ...
        # set paramters as property of class
        self.params = params

        ###### Step 1.0: Create PSF_r
        self.PSF_r = self.set_PSF_r()

        ###### Step 2.0: Create and store normalized data arrays.
        ...
        ###### Step 3.0: show some of the stored images from the data array to the user.
        if self.params.verbatim:
            show_random_img_plt_and_stats(self.lenses_array,    num_imgs=1, title="lenses")
            show_random_img_plt_and_stats(self.negatives_array, num_imgs=1, title="negatives")
            show_random_img_plt_and_stats(self.sources_array,   num_imgs=1, title="sources")


        ###### Step 3.1: Split the data into train and test data.
        self.X_train_lenses, self.X_test_lenses, self.y_train_lenses, self.y_test_lenses             = self.split_train_test_data(self.lenses_array,
                                                                                                                                0.0,
                                                                                                                                test_fraction=self.params.test_fraction)
        self.X_train_negatives, self.X_test_negatives, self.y_train_negatives, self.y_test_negatives = self.split_train_test_data(self.negatives_array,
                                                                                                                                0.0,
                                                                                                                                test_fraction=self.params.test_fraction)
        self.X_train_sources, self.X_test_sources, _, _                                              = self.split_train_test_data(self.sources_array,
                                                                                                                                "no_label", 
                                                                                                                                test_fraction=self.params.test_fraction)
        ...
        if self.params.verbatim:
            self.X_train_chunk, self.y_train_chunk = self.load_chunk(self.params.chunksize,
                                                                    self.X_train_lenses,
                                                                    self.X_train_negatives,
                                                                    self.X_train_sources,
                                                                    self.params.data_type,
                                                                    self.params.mock_lens_alpha_scaling)
            self.X_test_chunk, self.y_test_chunk   = self.load_chunk(self.params.validation_steps,
                                                                    self.X_test_lenses,
        ...
        if self.params.verbatim:
            print(self.y_train_chunk, flush=True)
            idxs_pos = np.where(self.y_train_chunk == 1.0)
            idxs_neg = np.where(self.y_train_chunk == 0.0)

            for i in range(25):
                pos_img = self.X_train_chunk[random.choice(list(idxs_pos[0]))]
                neg_img = self.X_train_chunk[random.choice(list(idxs_neg[0]))]
