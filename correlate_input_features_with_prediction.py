...
def plot_3D(con_fea1, con_fea2, con_fea3):
    ...
    plt.xlabel("Einstein Radius", fontsize=30)
    plt.ylabel("Source Intensity Scaling", fontsize=30)
    plt.title("Influence of brightness intensity scaling of Source and Einstein Radius", fontsize=30)
    plt.show()
...
def hexbin_plot(con_fea1, con_fea2, predictions, gridsize=10, calc_TPR=True):
    ...
    plt.xlabel("Einstein Radius", fontsize=30)
    plt.ylabel("Source Brighness Scaling", fontsize=30)
    cbar = plt.colorbar()
    cbar.ax.get_yaxis().labelpad = 15
    if calc_TPR:
        plt.title("Einstein Radius versus Brighness Scaling of Source - TPR (th={})".format(threshold), fontsize=30)
        cbar.ax.set_ylabel('True Positive Ratio per bin', rotation=270, fontsize=30)
    else:
        plt.title("Einstein Radius versus Brighness Scaling of Source and Model Certainty", fontsize=30)
        cbar.ax.set_ylabel('Model Prediction', rotation=270, fontsize=30)
    plt.show()
...
def show_scatterplot_ER_vs_intensity_and_certainty(einstein_radii, alpha_scalings, predictions):
    ...
    plt.xlabel("Einstein Radius", fontsize=30)
    plt.ylabel("Source Brighness Scaling", fontsize=30)
    plt.title("Einstein Radius and Brighness Scaling of Source versus Model Prediction", fontsize=30)
    cbar = plt.colorbar()
    cbar.ax.get_yaxis().labelpad = 15
    cbar.ax.set_ylabel('Prediction value Model', rotation=270, fontsize=30)
...
def plot_SNR_vs_TPR(predictions, SNRs, threshold, num_bins=10):
    ...
    plt.xticks(r, labels)
    plt.xlabel("Fraction of Source Image above 2x Average Noise in Lens", fontsize=30)
    plt.ylabel("Percentage", fontsize=30)
    plt.legend()
    plt.title("Percentage of True Positives and False Negatives per Bin", fontsize=30)# per binned (Source above 2x average noise) Interval")
...
def plot_feature_versus_prediction(predictions, feature_list, threshold=None, title=""):
    ...
    plt.title("{} vs network certainty. FNR: {:.2f}".format(title, len(preds_negatives)/sample_size), fontsize=30)
    plt.xlabel("{} of source".format(title), fontsize=30)
    plt.ylabel("Model prediction", fontsize=30)
    plt.legend()
    plt.show()
